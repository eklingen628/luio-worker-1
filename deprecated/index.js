...
var GoTrueClient = class _GoTrueClient {
  ...
  async _callRefreshToken(refreshToken2) {
    ...
    try {
      ...
      const { data, error } = await this._refreshAccessToken(refreshToken2);
      if (error)
        throw error;
      if (!data.session)
        throw new AuthSessionMissingError();
...
// src/sleep.ts
async function insertSleepData(supabase, data, dateQueried, user_id) {
  try {
...
__name(insertSleepData, "insertSleepData");

// src/user.ts
async function insertUserData(supabase, data) {
  try {
...
__name(insertUserData, "insertUserData");
async function getAllUserData(supabase) {
  try {
...
__name(getAllUserData, "getAllUserData");

// src/refresh.ts
async function refreshToken(userData, supabase, env) {
...
function calcTimeToRefresh(userData, interval) {
  const timeMilli = interval * 1.5 * 60 * 60 * 1e3;
  const dateTimeFuture = new Date(Date.now() + timeMilli);
  const expiry = new Date(userData.expires_at);
  return expiry < dateTimeFuture;
}
__name(calcTimeToRefresh, "calcTimeToRefresh");
...
// src/getData.ts
async function getData(data, config, dateQueried) {
  if (!dateQueried || !data) {
  ...
  try {
    ...
    console.log(JSON.stringify({
      ...
      status: err.details?.status ?? "unknown",
      message: err.message,
      details: err.details
      // optional; can omit if too noisy
    }));
    return null;
  }
  ;
  const dataFromQuery = await res?.json();
  return {
    dateQueried,
    dataFromQuery
  };
...
// src/activity.ts
async function insertActivityData(supabase, data, dateQueried, user_id) {
  try {
    ...
    if (Array.isArray(distances) && distances.length > 0) {
      for (const entry of distances) {
        ...
        const { error: error2 } = await supabase.from("daily_activity_distances").upsert({
          user_id,
          date_queried: dateQueried,
          activity,
          distance
...
__name(insertActivityData, "insertActivityData");

// src/heart.ts
async function insertHRTimeSeries(supabase, data, dateQueried, user_id) {
  try {
    const entries = data["activities-heart"];
    for (const entry of entries) {
      const { value } = entry;
      const { restingHeartRate, heartRateZones, customHeartRateZones } = value;
      const { data: summaryRow, error: summaryError } = await supabase.from("heart_activity_summary").upsert({
        user_id,
        date_queried: dateQueried,
        resting_heart_rate: restingHeartRate
      }, { onConflict: "user_id,date_queried" }).select("summary_id").single();
...
__name(insertHRTimeSeries, "insertHRTimeSeries");
...
// src/date.ts
function getQueryDate(refDate) {
  const milliFactor = 60 * 60 * 1e3;
  const nowUTC = new Date(Date.now());
  const baseDate = refDate ?? new Date(nowUTC.getTime() - 24 * milliFactor);
  const year = baseDate.getUTCFullYear();
  const month = String(baseDate.getUTCMonth() + 1).padStart(2, "0");
  const day = String(baseDate.getUTCDate()).padStart(2, "0");
  const dateQueried = `${year}-${month}-${day}`;
  return dateQueried;
}
__name(getQueryDate, "getQueryDate");
function genDates(startDate, endDate) {
  ...
  const milliConvert = 1e3 * 60 * 60 * 24;
...
var index_default = {
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/auth") {
      ...
      if (cookie !== null) {
        verifierString = cookie.split("; ").find((part) => part.startsWith("verifier="))?.split("=")[1];
      }
      if (!verifierString) {
